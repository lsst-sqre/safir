...
class ArqDependency:
    ...
    async def initialize(
        ...
        mode : `ArqMode`
            ...
            `ArqMode.production`, this method initializes a Redis-based arq
            queue and the dependency creates a `RedisArqQueue` client.

            With `ArqMode.test`, this method instead initializes an in-memory
            mocked version of arq that you use with the `MockArqQueue` client.
        redis_settings : `arq.connections.RedisSettings`
            The arq Redis settings, required when the ``mode`` is
            `ArqMode.production`. See arq's `~arq.connections.RedisSettings`
            documentationfor details on this object.
