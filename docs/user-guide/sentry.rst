...
When you want to handle Exceptions but still send notifications for them, Safir provides the `~safir.sentry.notify` function. This will notify Slack, or Sentry, or both, depending on which services are configured and initialized.
...
.. code-block:: python
   ...
   from safir.sentry import initialize_sentry, notify
   from safir.slack.blockkit import SlackException
   ...
   try:
       ...
       await notify(exc, slack_client=slack_client)
