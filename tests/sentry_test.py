...
from safir.sentry._helpers import notify
...
async def test_notify_both(
    ...
    await notify(exc, slack_client=client)
...
@pytest.mark.asyncio
async def test_notify_sentry_only(
    mock_slack: MockSlackWebhook, sentry_combo_items: Captured
) -> None:
    exc = SomeError(woo="hoo", bar="baz", user="someuser")

    await notify(exc, slack_client=None)
